...
export const HomeDataCard = ({ data }) => {
  ...
  return (
    <Box
      ...
      _hover={{
        shadow:
          "rgba(0, 0, 0, 0.25) 0px 2px 2px, rgba(0, 0, 0, 0.22) 0px 3px 3px;",
      }}
      onClick={(e) => {
    ...
    >
      ...
      <Flex m="20px 10px 10px 10px">
        <CircularProgress
          value={data.progress_circle}
          color="green"
          size="50px"
        >
          <CircularProgressLabel>{data.progress_circle}%</CircularProgressLabel>
        </CircularProgress>
      ...
      >
        <Text fontSize="13px" fontWeight="300" ml="10px">
          For every {data.status_msg ? data.status_msg : "$100"} you donate,
          Milaap will contribute $5 on your behalf.
        </Text>
      </Box>
    </Box>
  );
